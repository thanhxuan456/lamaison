...
# MAISON DELUXE Hotels & Resorts - Local File Installer
# Use this script when you have already downloaded the project from Replit.
#
# HOW TO USE:
#   1. Download the project ZIP from Replit (Three dots menu -> Download as zip)
#   2. Extract the ZIP anywhere on your Windows Server
#   3. Open PowerShell as Administrator
#   4. Run:  powershell -ExecutionPolicy Bypass -File "C:\path\to\local-install.ps1"
#
# The script auto-detects the source folder (same folder the script is in).
# No internet access is needed for the source code.
#
# OPTIONAL FLAGS:
...
#   5.  Copies source files from the script directory to the install directory
...
$Config = @{
    ...
    # -- DATABASE ----------------------------------------------------------
    # Option A: Neon cloud DB (recommended, no local PostgreSQL install needed)
    #   Fill NeonDatabaseUrl, leave UseLocalPg = $false
    ...
    # Example: "http://192.168.1.100:8080" or "http://myhotel.com:8080"
...
# ===========================================================
# STEP 5 -- COPY SOURCE FILES FROM LOCAL DIRECTORY
# ===========================================================
Write-Step "5/12" "Copying source files"

# The source is the directory that contains this script.
# When you download from Replit and extract the ZIP, put this script
# inside the extracted folder (it should already be there).
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Definition
Write-Info "Source : $scriptDir"
Write-Info "Target : $($Config.InstallDir)"

# Verify this is the correct source
$schemaCheck = Join-Path $scriptDir "lib\db\src\schema\index.ts"
if (-not (Test-Path $schemaCheck)) {
    Fail @"
Source validation failed: lib\db\src\schema\index.ts not found in:
  $scriptDir

Make sure you are running this script from inside the extracted project folder.
The folder should contain: artifacts\, lib\, package.json, pnpm-workspace.yaml
"@
}
...
if ($scriptDir -ne $Config.InstallDir) {
    ...
    Write-Info "Script is already running inside the install directory -- skipping copy"
}
...
# VITE_API_URL is always empty so the browser uses relative /api/... paths.
# The serve-frontend.mjs proxy forwards them to the API internally (no CORS).
$envLines = @(
    "NODE_ENV=production",
...
# ===========================================================
# STEP 9 -- INSTALL, PUSH DB SCHEMA, SEED, BUILD
# ===========================================================
...
try {
    ...
    if (Test-Path $nmDir) {
        Write-Info "Removing root node_modules for a clean Windows install..."
    ...
    if ([string]::IsNullOrEmpty($DatabaseUrl)) {
        Fail "DATABASE_URL is empty -- check NeonDatabaseUrl in the Config block."
    }

    & pnpm --filter "@workspace/db" run push
    if ($LASTEXITCODE -ne 0) { Fail "Database schema push failed. Verify your database URL and network connectivity." }
...
# Build the serve-frontend.mjs static file server.
# It serves the built frontend and proxies /api/* to the API server internally.
# API_BACKEND_URL (set in the service env below) tells it where to forward requests.
