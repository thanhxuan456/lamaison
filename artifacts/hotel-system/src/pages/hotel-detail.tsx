...
import { MapPin, Star, Wifi, Coffee, Wind, Monitor, ChevronRight, Check } from "lucide-react";
...
export default function HotelDetail() {
    ...
    return (
      <PageLayout>
        <div className="min-h-screen flex flex-col items-center justify-center text-center px-4">
          <h1 className="text-4xl font-serif text-secondary mb-4">Không tìm thấy khách sạn</h1>
          <p className="text-muted-foreground mb-8">Khách sạn bạn đang tìm kiếm không tồn tại hoặc đã bị gỡ.</p>
  ...
  // Choose appropriate hero image based on city
        ...
        <div className="container mx-auto px-4 md:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
            <div className="lg:col-span-2">
              <h2 className="text-primary font-serif tracking-[0.2em] text-sm uppercase mb-4">Giới Thiệu</h2>
              <h3 className="text-3xl font-serif text-secondary mb-6">Đỉnh Cao Của Sự Thanh Lịch</h3>
                ...
                {hotel.amenities.map((amenity, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex flex-col items-center text-center p-4 border border-primary/10 bg-secondary/5">
                      ...
                      <span className="text-sm font-medium text-secondary">{amenity}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div>
              <div className="bg-secondary p-8 border border-primary/30 text-white relative">
                ...
                <h4 className="font-serif text-xl mb-6 border-b border-primary/20 pb-4">Thông tin đặt phòng</h4>
                
                <div className="space-y-6">
                  <div>
                    <span className="text-white/60 text-sm block mb-1">Giá từ</span>
                    <span className="text-primary text-3xl font-serif">${hotel.priceFrom}</span>
                    <span className="text-white/60 text-sm"> / đêm</span>
                  </div>
                  
                  {!loadingSummary && summary && (
                    <>
                      <div className="flex justify-between items-center border-b border-primary/10 pb-4">
                        <span className="text-white/70 font-light">Phòng trống</span>
                        <span className="text-white font-medium">{summary.availableRooms} / {summary.totalRooms}</span>
                      </div>
                      <div className="flex justify-between items-center border-b border-primary/10 pb-4">
                        <span className="text-white/70 font-light">Đánh giá</span>
                  ...
                  <div className="pt-4">
                    <Button asChild className="w-full bg-primary text-primary-foreground hover:bg-primary/90 rounded-none py-6 tracking-widest uppercase">
                      <Link href={`/hotels/${hotel.id}/rooms`}>Xem Phòng Trống</Link>
                    </Button>
                  </div>
                </div>
              </div>
      ...
      <section className="py-24 bg-background">
        <div className="container mx-auto px-4 md:px-8">
          <div className="flex justify-between items-end mb-12">
            <div>
              <h2 className="text-primary font-serif tracking-[0.2em] text-sm uppercase mb-4">Lưu Trú</h2>
              <h3 className="text-3xl md:text-4xl font-serif text-secondary">Phòng & Suite Ngoại Hạng</h3>
            </div>
            <Link href={`/hotels/${hotel.id}/rooms`} className="hidden md:flex items-center text-primary text-sm uppercase tracking-widest font-medium hover:text-primary/80 transition-colors group">
              Xem tất cả <ChevronRight size={16} className="ml-1 transition-transform group-hover:translate-x-1" />
            </Link>
          </div>
          ...
          {loadingRooms ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {rooms?.slice(0, 2).map((room) => (
                <div key={room.id} className="group border border-primary/20 flex flex-col md:flex-row bg-white overflow-hidden">
                  ...
                  <div className="w-full md:w-1/2 p-8 flex flex-col justify-between">
                    <div>
                      <h4 className="font-serif text-2xl text-secondary mb-2">{room.type}</h4>
                      <p className="text-muted-foreground text-sm font-light mb-4 line-clamp-2">{room.description}</p>
                      <div className="flex flex-wrap gap-x-4 gap-y-2 mb-6">
                        <span className="text-xs text-secondary/70 uppercase tracking-wider flex items-center gap-1">
                          <User size={12} className="text-primary" /> {room.capacity} Khách
                        </span>
                        <span className="text-xs text-secondary/70 uppercase tracking-wider flex items-center gap-1">
                    ...
                    <div>
                      <div className="text-2xl font-serif text-primary mb-4">${room.pricePerNight}<span className="text-sm text-muted-foreground font-sans">/đêm</span></div>
          ...
          <div className="mt-8 text-center md:hidden">
            <Button asChild variant="link" className="text-primary uppercase tracking-widest">
              <Link href={`/hotels/${hotel.id}/rooms`}>Xem tất cả phòng</Link>
            </Button>
          </div>
        </div>
      </section>
