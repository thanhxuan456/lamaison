    ...
    <PageLayout>
      ...
      <section className="bg-secondary pt-32 pb-16 border-b border-primary/20">
        <div className="container mx-auto px-4 md:px-8 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-serif text-white mb-6">Phòng & Suite</h1>
          <div className="w-16 h-[2px] bg-primary mx-auto mb-6"></div>
          <p className="text-white/70 font-light max-w-2xl mx-auto">
            Không gian lưu trú được thiết kế tinh xảo, kết hợp hài hòa giữa nét đẹp truyền thống và tiện nghi hiện đại bậc nhất.
          </p>
        </div>
      </section>
            ...
            {rooms?.map((room, index) => (
              <div key={room.id} className="flex flex-col md:flex-row gap-8 items-stretch group">
                ...
                <div className={`w-full md:w-1/2 flex flex-col justify-center py-4 ${index % 2 !== 0 ? 'md:order-1 items-end text-right' : 'items-start text-left'}`}>
                  <h3 className="text-3xl font-serif text-secondary mb-4">{room.type}</h3>
                  <div className={`flex flex-wrap gap-4 text-sm text-muted-foreground uppercase tracking-widest mb-6 ${index % 2 !== 0 ? 'justify-end' : 'justify-start'}`}>
                    <span className="flex items-center gap-1"><User size={14} className="text-primary"/> {room.capacity} Khách</span>
                    <span className="flex items-center gap-1"><Wind size={14} className="text-primary"/> {room.view}</span>
                    <span className="flex items-center gap-1"><Maximize size={14} className="text-primary"/> Tầng {room.floor}</span>
                  </div>
                  ...
                  <div className="text-3xl font-serif text-primary mb-8">
                    ${room.pricePerNight}<span className="text-sm font-sans text-muted-foreground">/đêm</span>
                  </div>
